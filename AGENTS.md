...
# 10 · Agent Notepad (notes.md)

* This repo may contain an agent notepad at 'notes.md' if not create one in the root.
* You may append notes to `notes.md` at any time during a task when it will help future work (decisions, discoveries, gotchas, constraints, TODOs).
* After completing each task (i.e., at the end of a run), you must append a short entry to `notes.md` with:
  * A title that captures the core idea of the task;
  * 2-6 bullet points summarizing what you did and what the next person needs to know (key changes, why, verification steps, and any follow-ups/risks).
* The notepad is **append-only**:
  * Do not delete, rewrite, or “clean up” prior entries.
  * Only add a new entry at the end.
* Keep entries concise and in English. Do not include secrets or sensitive tokens.
